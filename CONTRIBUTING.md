...
5. Build locally to confirm the image still builds and runs:
   ```bash
   docker buildx build -f Dockerfile-multi-arch -t alpine-3.23-s6 --load .
   docker run -it --rm alpine-3.23-s6
   ```
6. Check that s6 still behaves as expected:
   ```bash
   # s6 must be PID 1
   docker run --rm alpine-3.23-s6 sh -c 'cat /proc/1/comm'   # s6-svscan
   # PUID/PGID must be applied at runtime
   docker run --rm -e PUID=1500 -e PGID=1600 alpine-3.23-s6 sh -c 'id appuser'
   ```
